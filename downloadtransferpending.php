...
header('Content-Disposition: attachment; filename=vamt_icici_transferpending.csv');

// create a file pointer connected to the output stream/
$output = fopen('php://output', 'w');
//Asset Code
// output the column headings
fputcsv($output, array('Transfer Location','Previous Location','Asset Code','IP address', 'Asset Type', 'FCRM No.', 'ATF No','Store Confirmation','TIG Approved','Courier Company Name','Docket No.','Transit Insurance','Asset Status','Status Remark','Serial','Group')); 
 ...
 $ExtraFields	=	array(
    ...
    $query = "SELECT glpi_transfer.* , glpi_groups.`name` as groupname,$typestable[$i].`name` as typename,glpi_states.`name` as statesname,glpi_plugin_fields_statusremarkfielddropdowns.`name` as remarkname,glpi_transfer.`fcrmno` as fcrmnumber,glpi_transfer.`atfno` as atfno ,$ExtraFields[$i],$tables[$i].serial as serialname,$tables[$i].name as name  FROM glpi_transfer
                ...
                LEFT JOIN `glpi_plugin_fields_statusremarkfielddropdowns`
                    ...
                    where status=0  and assettype='".$types."'".$groupqry;
      ...
      if ($result->numrows() > 0) {
      	foreach ($result as $line) {
			...
			fputcsv($output, array($newlocation,$oldlocation,$line['name'],$line['ipaddress'],$line['typename'],$line['fcrmno'],$line['atfno'],$store,$tig,$line['couriercompanyname'],$line['docketno'],$tin,$line['statesname'],$line['remarkname'],$line['serialname'],$line['groupname']));      		
      	}
      }
 	}
 ?>